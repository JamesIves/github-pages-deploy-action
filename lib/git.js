...
export async function deploy(action) {
    ...
    try {
        ...
        await execute(`rsync -q -av --checksum --progress ${isMkpathSupported && action.targetFolder ? '--mkpath' : ''} ${action.folderPath}/. ${action.targetFolder
            ...
            ? `--delete ${excludes} ${!fs.existsSync(`${action.folderPath}/${DefaultExcludedFiles.CNAME}`)
                ...
                : ''}`
            : ''}  --exclude ${DefaultExcludedFiles.SSH} --exclude ${DefaultExcludedFiles.GIT} --exclude ${DefaultExcludedFiles.GITHUB} ${action.folderPath === action.workspace
