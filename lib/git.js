        ...
        try {
            ...
            try {
                if ((process.env.CI && !action.sshKey) || action.isTest) {
                    ...
                    */
                    const includeIfResult = yield (0, execute_1.execute)(`git config --local --get-regexp 'includeIf\\..*\\.path'`, action.workspace, true // Always silent to avoid exposing credential paths
                    );
                    ...
                    if (includeIfResult.stdout) {
                        ...
                        for (const line of lines) {
                            ...
                            if (match) {
                                ...
                                try {
                                    yield (0, execute_1.execute)(`git config --local --remove-section "${section}"`, action.workspace, true // Always silent
                                    );
                                }
                                catch (_c) {
