...
function init(action) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (util_1.isNullOrUndefined(action.accessToken) &&
                util_1.isNullOrUndefined(action.gitHubToken) &&
                util_1.isNullOrUndefined(action.ssh)) {
            ...
            yield execute_1.execute(`git config user.name ${action.name}`, constants_1.workspace);
            yield execute_1.execute(`git config user.email ${action.email}`, constants_1.workspace);
...
function switchToBaseBranch(action) {
    return __awaiter(this, void 0, void 0, function* () {
        yield execute_1.execute(`git checkout --progress --force ${action.baseBranch ? action.baseBranch : action.defaultBranch}`, constants_1.workspace);
...
function generateBranch(action) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (util_1.isNullOrUndefined(action.branch)) {
                throw Error("Branch is required.");
            }
            console.log(`Creating ${action.branch} branch... 🔧`);
            yield switchToBaseBranch(action);
            yield execute_1.execute(`git checkout --orphan ${action.branch}`, constants_1.workspace);
            yield execute_1.execute(`git reset --hard`, constants_1.workspace);
            yield execute_1.execute(`git commit --allow-empty -m "Initial ${action.branch} commit."`, constants_1.workspace);
            yield execute_1.execute(`git push ${constants_1.repositoryPath} ${action.branch}`, constants_1.workspace);
            yield execute_1.execute(`git fetch`, constants_1.workspace);
        }
        catch (error) {
...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        /*
          ...
          */
        const branchExists = yield execute_1.execute(`git ls-remote --heads ${constants_1.repositoryPath} ${action.branch} | wc -l`, constants_1.workspace);
        ...
        yield execute_1.execute(`git worktree add --checkout ${temporaryDeploymentDirectory} origin/${action.branch}`, constants_1.workspace);
        ...
        yield execute_1.execute(`rsync -q -av --progress ${action.build}/. ${action.targetFolder
            ? `${temporaryDeploymentDirectory}/${action.targetFolder}`
            : temporaryDeploymentDirectory} ${action.clean
            ? `--delete ${excludes} --exclude CNAME --exclude .nojekyll`
            : ""}  --exclude .ssh --exclude .git --exclude .github ${action.build === constants_1.root ? `--exclude ${temporaryDeploymentDirectory}` : ""}`, constants_1.workspace);
        ...
        yield execute_1.execute(`git commit -m "${!util_1.isNullOrUndefined(action.commitMessage)
            ...
            : `Deploying to ${action.branch} from ${action.baseBranch}`} - ${process.env.GITHUB_SHA} 🚀" --quiet`, temporaryDeploymentDirectory);
        yield execute_1.execute(`git push --force ${constants_1.repositoryPath} ${temporaryDeploymentBranch}:${action.branch}`, temporaryDeploymentDirectory);
        ...
        yield execute_1.execute(`git checkout --progress --force ${action.defaultBranch}`, constants_1.workspace);
